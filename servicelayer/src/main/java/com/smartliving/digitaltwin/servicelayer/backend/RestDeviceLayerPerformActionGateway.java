...
public class RestDeviceLayerPerformActionGateway implements InvokeActionCommandOutPort {
    ...
    public Mono<SLInteractionResponse> invokeAction(SLInteractionRequest request) {
        return webClient.put().uri(URI.create(uri+path)).accept(MediaType.APPLICATION_JSON)
                ...
                .onErrorResume(throwable -> Mono.just(SLInteractionResponse.builder().id(request.getId()).status(new Document().append("error", throwable.getCause())).build()));
    }
}
